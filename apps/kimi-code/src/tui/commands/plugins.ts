...
function showPluginInstallResult(
  ...
  host.showStatus(
    `${action} (${summary.id}).${mcpHint} Run /reload or /new to apply plugin changes.`,
  );
  if (options?.successNotice === 'marketplace') {
    host.showNotice(
      `Installed or updated ${summary.displayName}`,
      `Marketplace install or update succeeded for ${summary.id}. Run /reload or /new to apply plugin changes.`,
    );
  }
}
