...
| `/probemodels` | — | Probe all configured models to check API reachability. Shows a summary of healthy and unreachable models. Probe status (● healthy, ◐ unknown, ● error) is also displayed in the model picker. Models are probed automatically at session start and re-probed after provider errors. | Yes |
