...
| `/spiceup` | — | 覆盖当前会话的模型采样参数（temperature、top_p、top_k、max_tokens、frequency_penalty、presence_penalty）。打开选择器以逐项开关参数。覆盖立即生效，优先于 config.toml 默认值，并被子 Agent 继承。 | 是 |
| `/probemodels` | — | 探测所有已配置模型的 API 可达性。显示健康和不可达模型的摘要。探测状态（● 健康 / ◐ 未知 / ● 错误）也会显示在模型选择器中。模型在会话启动时、第一次子 Agent 启动前自动探测，并在提供商错误后自动重新探测。 | 是 |
| `/forcemcp [on\|off]` | — | 切换 force-MCP 模式。开启后所有已连接的 MCP 工具都会被激活，并提醒模型只使用对当前任务有补充上下文的工具。使用 `/forcemcp off`（或再次切换）可恢复原来的激活工具集。 | 是 |
| `/criticize` | `/critique` | 启动一个专门的批评子 Agent，分析主 Agent 工作中的缺陷、幻觉、遗漏边界情况和替代方案。批评子 Agent 会携带与主 Agent 相关的记忆启动，批评结果会作为 `critique-finding` 记忆持久化到未来会话。批评结果也会注入回上下文，让主 Agent 审阅、辩驳或接受每一条意见。在 Plan 模式下会先获取当前计划内容供分析。如果尚未配置批评模型，会先弹出模型选择对话框。当启用 `auto-critique-checkpoints` 实验功能后，批评子 Agent 还会在计划审批、多文件编辑和目标完成前自动运行。 | 是 |
| `/compare [<prompt>]` | `/ab` | 用 2-4 个模型并行跑同一提示词，并在结果面板中并排查看各模型回复。未提供提示词时，使用最近一条用户消息。在结果面板中按 `1`-`4` 可将对应回复提升为主上下文用户消息，按 `S` 可让主 Agent 综合各回复优点，按 `Esc`/`Q` 关闭面板。提升或综合的选择会作为 `decision` 记忆保存，同时比较摘要会作为 `comparison` 记忆保存。 | 是 |
| `/memory [list]` | — | 打开所有已记住记忆的滚动列表（全局 + 项目）。显示 id、内容、分类、标签和置顶状态。用 `↑`/`↓` 浏览，`P` 置顶/取消置顶，`D` 删除，`Esc`/`Q` 关闭。 | 是 |
| `/memory delete <id>` | — | 按 id 删除指定记忆。 | 是 |
| `/memory pin <id>` | — | 将记忆标记为始终注入，无论当前查询相关性如何都会召回。 | 是 |
| `/memory unpin <id>` | — | 取消记忆的始终注入标记。 | 是 |
| `/reloadsysprompt` | `/reload-sys-prompt` | 在不重启会话的情况下，从 `.kimi-code/sysprompt.md`（或全局覆盖文件）重新加载系统提示词。适合在应用运行时编辑系统提示词后热重载。 | 是 |
| `/goal [...]` | — | 开始或管理目标模式 | 见下文 |
...
当 `Memory` 工具启用时，Kimi Code CLI 会在每次 Assistant 轮次完成后主动提取 durable facts（可持续记忆），例如用户偏好、项目事实和已确认的决策。提取现在基于 LLM：当前会话模型读取最近的用户/助手对话并输出最多 3 条建议记忆的 JSON。轮次结束后，它会提出这些记忆。在 `auto` 或 `YOLO` 权限模式下，记忆会被静默保存；在 `manual` 模式下，会弹出一个临时的 TUI 提示，让你逐条批准或跳过。已保存的记忆会在后续轮次中根据当前查询的相关性自动召回并排序，置顶记忆无论相关性如何都会被注入。你也可以通过 `Memory` 工具或 `/memory` 斜杠命令显式地管理记忆。
