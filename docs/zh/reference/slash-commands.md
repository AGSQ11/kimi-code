...
| `/criticize` | `/critique` | 启动一个专门的批评子 Agent，分析主 Agent 工作中的缺陷、幻觉、遗漏边界情况和替代方案。批评结果会注入回上下文，让主 Agent 审阅、辩驳或接受每一条意见。在 Plan 模式下会先获取当前计划内容供分析。如果尚未配置批评模型，会先弹出模型选择对话框。当启用 `auto-critique-checkpoints` 实验功能后，批评子 Agent 还会在计划审批、多文件编辑和目标完成前自动运行。 | 是 |
| `/compare [<prompt>]` | `/ab` | 用 2-4 个模型并行跑同一提示词，并在结果面板中并排查看各模型回复。未提供提示词时，使用最近一条用户消息。在结果面板中按 `1`-`4` 可将对应回复提升为主上下文用户消息，按 `S` 可让主 Agent 综合各回复优点，按 `Esc`/`Q` 关闭面板。 | 是 |
