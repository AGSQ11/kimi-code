...
export async function prepareSystemPromptContext(
  ...
  const [cwdListing, agentsMdResult, additionalDirsInfo, systemPromptOverride] = await Promise.all([
...
function renderAgentFiles(files: readonly AgentFile[]): string {
  ...
  // Upstream no longer truncates AGENTS.md content; it only reports a warning
  // when the combined size exceeds the recommended budget. Render everything.
