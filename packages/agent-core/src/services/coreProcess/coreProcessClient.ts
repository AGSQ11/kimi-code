 ...
 *   requestMemoryApproval   → auto-approve all proposed memories (daemon has no TUI)
...
import type { ApprovalRequest, ApprovalResponse, Event, MemoryApprovalRequest, MemoryApprovalResponse, QuestionRequest, QuestionResult, SDKAPI, ToolCallRequest, ToolCallResponse } from '../../rpc';
...
export class BridgeClientAPI implements SDKAPI {
  ...
  async requestMemoryApproval(
    ...
    // The daemon adapter has no interactive memory-approval UI. Auto-approve
    // every proposed memory so server-mode sessions still benefit from the
    // memory loop.
    return { approved: request.memories.map((_, index) => index) };
  }
