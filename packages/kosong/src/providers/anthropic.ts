...
export interface AnthropicOptions {
  ...
  /**
   * Explicitly declare whether the model supports any thinking/reasoning
   * parameter. When false, `withThinking('off')` will omit the `thinking`
   * field from the request body instead of sending `{ type: 'disabled' }`,
   * which some Anthropic-compatible endpoints reject. Defaults to inferring
   * from the model name via the capability registry.
   */
...
export class AnthropicChatProvider implements ChatProvider {
  ...
  private _thinkingSupported: boolean | undefined;
  ...
  constructor(options: AnthropicOptions) {
    ...
    this._thinkingSupported = options.thinkingSupported;
  ...
  async generate(
    ...
    // Fable and other thinking-unsupported backends reject an explicit
    // `disabled` thinking config (HTTP 400, unlike Opus 4.7/4.8 which accept
    // it), so omit the field instead. Note thinking cannot actually be turned
    // off on Fable: adaptive thinking is always on, and an omitted `thinking`
    // field still runs with it.
    const thinking = this._generationKwargs.thinking;
    if (thinking !== undefined) {
      const thinkingSupported =
        this._thinkingSupported ?? getAnthropicModelCapability(this._model).thinking;
      const shouldOmitDisabled =
        thinking.type === 'disabled' && (isFableModel(this._model) || !thinkingSupported);
      if (!shouldOmitDisabled) {
